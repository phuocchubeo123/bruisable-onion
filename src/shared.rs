...
impl IntermediaryNode {
    // Constructor method to create a new IntermediaryNode, generating its own X25519 key pair
    pub fn new(id: &str) -> Self {
        let mut rng = OsRng; // Secure RNG
        let secret_key = StaticSecret::new(&mut rng); // Generate private key (StaticSecret)
        let public_key = X25519PublicKey::from(&secret_key); // Derive public key from private key
    ...
    // // Method to append the public key to the PKKeys.txt file
    // pub fn append_pubkey_to_file(&self) -> io::Result<()> {
    //     // Open the PKKeys.txt file in append mode
    //     let file_path = "PKKeys.txt";
    //     let mut file = OpenOptions::new()
    //         .append(true)
    //         .create(true)  // If the file doesn't exist, create it
    //         .open(file_path)?;

    //     // Write the ID and public key in Base64 to the file
    //     writeln!(file, "ID: {}", self.id)?;
    //     writeln!(file, "Public Key: {}", base64::encode(self.public_key.as_bytes()))?;

    //     Ok(())
    // }
    
    //more error handling than above code
    pub fn append_pubkey_to_file(&self) -> io::Result<()> {
        let file_path = "PKKeys.txt";
        ...
        // Write the ID and public key in Base64 to the file
        writeln!(file, "ID: {}", self.id)
            .map_err(|e| io::Error::new(e.kind(), "Failed to write ID to the file"))?;
        writeln!(file, "Public Key: {}", base64::encode(self.public_key.as_bytes()))
            .map_err(|e| io::Error::new(e.kind(), "Failed to write public key to the file"))?;
        writeln!(file)?; // Add a blank line for readability
