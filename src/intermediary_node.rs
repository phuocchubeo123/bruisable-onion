...
/// Represents an intermediary node with its own key pair and unique ID.
...
impl IntermediaryNode {
    // This method creates a new IntermediaryNode and returns it
...
pub fn register_node(nodes: &mut HashMap<String, IntermediaryNode>, id: &str, public_key: RsaPublicKey, private_key: RsaPrivateKey) {
    let node = IntermediaryNode::new(id, public_key, private_key);
    nodes.insert(id.to_string(), node);
}

    // pub fn public_key_pem(&self) -> String {
    //     self.public_key.to_pkcs1_pem(LineEnding::LF).expect("Failed to encode public key to PEM")
    // }
    
    // pub fn private_key_pem(&self) -> String {
    //     self.private_key.to_pkcs1_pem(LineEnding::LF).expect("Failed to encode private key to PEM").to_string()
    // }

    // /// Returns the private key (used for decryption within the node).
